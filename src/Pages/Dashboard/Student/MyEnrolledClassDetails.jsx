...
const MyEnrolledClassDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchAssignments = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(`https://edumanage-server-rho.vercel.app/assignments?classId=${classId}`);
  ...
  const handleSubmit = async (assignmentId) => {
    ...
    try {
      ...
      const res = await fetch(`https://edumanage-server-rho.vercel.app/submissions`, {
        method: 'POST',
  ...
  const handleSendTER = async () => {
    ...
    try {
      const res = await fetch('https://edumanage-server-rho.vercel.app/feedbacks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          classId,
